...
void DotSceneLoader::parseDotScene(const String &SceneName, const String &groupName, SceneManager *yourSceneMgr, SceneNode *pAttachNode, const String &sPrependNode)
{
	...
	try
	{
		...
		if( XMLDoc->Error() )
		{
			...
			LogManager::getSingleton().logMessage(XMLDoc->ErrorDesc()) ;
			delete XMLDoc;
			return;
		}
	}
...
void DotSceneLoader::processCamera(TiXmlElement *XMLNode, SceneNode *pParent)
{
	...
	Real aspectRatio = Ogre::Real(getAttribReal(XMLNode, "aspectRatio", 1.3333));
...
void DotSceneLoader::processEntity(TiXmlElement *XMLNode, SceneNode *pParent)
{
	...
	// Process subentities for material name change
	processSubEntities(XMLNode, pEntity);
...
{
	// Process attributes
	Real expDensity = Ogre::Real(getAttribReal(XMLNode, "expDensity", 0.001));
...
{
	if(pEntity!=NULL && XMLNode!=NULL)
	{
		TiXmlElement * subEntities = XMLNode->FirstChildElement("subentities") ;
		if(subEntities!=NULL)
		{
			TiXmlElement * entity = subEntities->FirstChildElement("subentity") ;
			while(entity)
			{
				int index = atoi(getAttrib(entity, "index", "0").c_str()) ;
				String materialName = getAttrib(entity, "materialName", "") ;
				if(materialName!="")
				{
					pEntity->getSubEntity(index)->setMaterialName(materialName) ;
				}
				entity = entity->NextSiblingElement("subentity") ;
			}
		}
	}
}